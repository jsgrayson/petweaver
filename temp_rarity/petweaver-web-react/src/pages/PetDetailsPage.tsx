...
export const PetDetailsPage: React.FC = () => {
  ...
  const rarityClass = pet ? rarityColor[pet.rarity] : '';
  const auraClass = pet ? rarityGlow[pet.rarity] : '';
    ...
    >
      <div className="page-bg-inner space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-amber-50 flex items-center gap-2">
            {pet && (
              <>
                <img
                  ...
                  className="h-4 w-4"
                />
                <span className={rarityClass}>{pet.rarity}</span>
                <span>•</span>
              </>
            )}
            {pet ? pet.name : `Pet #${id}`}
          ...
          <Card className="bg-bg-panel/85">
            <div className="flex gap-4">
              <div className="relative flex-shrink-0">
                <div className={`h-32 w-32 rounded-full bg-gradient-to-tr from-ember-deep via-ember-light to-arcane-mid shadow-ember flex items-center justify-center overflow-hidden ${auraClass}`}>
                  <img
                    src={portraitSrc}
